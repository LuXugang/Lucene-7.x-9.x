...
public class SoftDeletesTest2 {
    ...
    public void doIndexAndSearch() throws Exception {
        ...
        indexWriter.softUpdateDocument(new Term("abc", "document1"), newDoc, new NumericDocValuesField("softDeleteField", 3));
        // 文档3
        doc = new Document();
        doc.add(new StringField("abc", "document1", Field.Store.YES));
        indexWriter.addDocument(doc);
        ...
        newDoc = new Document();
        newDoc.add(new StringField("abc", "document3", Field.Store.YES));
        indexWriter.softUpdateDocument(new Term("abc", "document1"), newDoc, new NumericDocValuesField("softDeleteField", 4));
        indexWriter.commit();
