...
public class FacetsConfig {
    ...
    public static final class DimConfig {
        ...
        // 一个dim的label是否可以是多值的, 例：new FacetField("brand", "brand1", "brand2", "小米")
        public boolean hierarchical;

        // 同一个dim是否可以多个FacetField
    ...
    public Document build(TaxonomyWriter taxoWriter, Document doc) throws IOException {
        ...
        // key是dim值，value是 SortedSetDocValuesFacetField对象
        ...
        // 这个过程是根据不同类型域类型，将不同document的类型相同的域放到对应的Map对象(就是上文定义的三个Map对象)中, 目的是相同类型的域用相同的处理方法(跟属于document无关)
        for (IndexableField field : doc) {
            ...
            if (field.fieldType() == SortedSetDocValuesFacetField.TYPE) {
                ...
                // 根据key值(dim的值)取出这个dim对应的配置(这些配置是之前我们通过FacetsConfig中的一些方法设置的)
                FacetsConfig.DimConfig dimConfig = getDimConfig(facetField.dim);
                // 判断是不是多值的
                if (dimConfig.multiValued == false) {
                    checkSeen(seenDims, facetField.dim);
        ...
        // 将不属于FacetField.TYPE 跟SortedSetDocValuesFacetField.TYPE 跟 AssociationFacetField.TYPE中任何一个的直接塞到新的document中
        ...
        for(Map.Entry<String,List<FacetField>> ent : byField.entrySet()) {
            ...
            for(FacetField facetField : ent.getValue()) {
                ...
                if (ft.multiValued && (ft.hierarchical || ft.requireDimCount)) {
                    ...
                    // 取出所有的parent的ordinal值，存放到ordinals数组中
                    while (parent > 0) {
                ...
                // 将component中的不同长度的前缀添加到索引中(因为任意一个前缀都可能是其他category的前缀，也是了能进行统计)
    ...
    private void processSSDVFacetFields(Map<String,List<SortedSetDocValuesFacetField>> byField, Document doc) throws IOException {
        ...
        for(Map.Entry<String,List<SortedSetDocValuesFacetField>> ent : byField.entrySet()) {
            ...
            for(SortedSetDocValuesFacetField facetField : ent.getValue()) {
                // 创建一个FacetLabel对象后，会将dim跟label的信息写到一个component数组里面
                FacetLabel cp = new FacetLabel(facetField.dim, facetField.label);
                // 将component数组中的值合并成一个String，用特殊字符"\u001F"来区分component中的不同数据
                String fullPath = pathToString(cp.components, cp.length);
                //System.out.println("add " + fullPath);

                // For facet counts:
                // 生成SortedSetDocValues对象
                doc.add(new SortedSetDocValuesField(indexFieldName, new BytesRef(fullPath)));

                // For drill-down:
                // 注意这里是Field.Stroe.NO, 并且是对 fullPath进行dirll-down
                doc.add(new StringField(indexFieldName, fullPath, Field.Store.NO));

                FacetsConfig.DimConfig ft = getDimConfig(facetField.dim);
                // if语句为true：则是对 dim进行drill-dwon
                if (ft.requireDimensionDrillDown) {
    ...
    // int类型的压缩, 最少使用一个字节存储，最多5个字节存储int类型数值
