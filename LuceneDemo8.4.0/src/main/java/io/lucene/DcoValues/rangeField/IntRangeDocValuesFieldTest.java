...
public class IntRangeDocValuesFieldTest {
    ...
    public void doIndexAndSearch() throws Exception {
        ...
        int[] max3 = {10, 10};
        // 第三个RangeField
        doc.add(new IntRangeDocValuesField("level", min3, max3));
        indexWriter.addDocument(doc);
        // 文档3
        doc = new Document();
        int[] min4 = {9, 6};
        int[] max4 = {10, 7};
        // 第四个RangeField
        doc.add(new IntRangeDocValuesField("level", min4, max4));
        indexWriter.addDocument(doc);
        DirectoryReader reader = DirectoryReader.open(indexWriter);
        IndexSearcher searcher = new IndexSearcher(reader);
        final int[] lowQueryRange = {2, 0};
        final int[] highQueryRange = {8, 8};
        Query query = IntRangeDocValuesField.newSlowIntersectsQuery("level", lowQueryRange, highQueryRange);
