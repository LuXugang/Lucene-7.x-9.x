...
/**
 * @author Lu Xugang
 * @date 2020/6/28 2:19 下午
 */
public class SoftDeletesTest6 {
    ...
    public void doIndexAndSearch() throws Exception {
        ...
        indexWriter.softUpdateDocument(new Term("abc", "abc"), newDoc, new NumericDocValuesField("softDeleteField", 3));
        // 第二次软删除
        newDoc = new Document();
        newDoc.add(new StringField("abc", "document4", Field.Store.YES));
        indexWriter.softUpdateDocument(new Term("abc", "ab"), newDoc, new NumericDocValuesField("softDeleteField", 1));
        // 第三次软删除
        newDoc = new Document();
        newDoc.add(new StringField("abc", "document5", Field.Store.YES));
        indexWriter.softUpdateDocument(new Term("abc", "abd"), newDoc, new NumericDocValuesField("softDeleteField", 2));
        indexWriter.commit();
    ...
    public static void main(String[] args) throws Exception{
        SoftDeletesTest6 test = new SoftDeletesTest6();
